...
def compute_geographic_similarity(
    ...
    """Computes geographic similarity using Haversine coordinate decay or location text fallback."""
    w = settings.SIM_WEIGHT_GEOGRAPHIC

    g1 = c1.geographic
    g2 = c2.geographic

    # Scenario A: GPS Coordinates Available on Both Cases
    if g1.latitude is not None and g1.longitude is not None and g2.latitude is not None and g2.longitude is not None:
        ...
        return CaseSimilaritySignal(
            ...
            status=status,
            evidence=evidence,
            explanation=f"Haversine GPS distance: {dist_km:.2f} km with decay factor {decay_km} km."
        )

    # Scenario B: Text Locality Fallback
    ...
    if dist1 and dist2:
        if dist1.strip().lower() == dist2.strip().lower():
            return CaseSimilaritySignal(
                ...
                explanation=f"District match: {dist1}"
            )
        else:
            return CaseSimilaritySignal(
                signal_name="GEOGRAPHIC_SIMILARITY",
    ...
    return CaseSimilaritySignal(
        ...
        evidence=[],
        explanation="Geographic information unavailable on one or both cases."
    )
