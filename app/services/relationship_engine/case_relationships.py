    ...
    if mo_sig.status in (SignalStatus.MATCH, SignalStatus.PARTIAL):
        signals.append(
            RelationshipSignal(
                ...
                certainty=SignalCertainty.INFERRED_PATTERN,
                evidence=mo_sig.evidence,
                explanation=mo_sig.explanation,
                supporting_entity_ids=[],
                provenance="Step 4B MO Text Similarity",
                uncertainty_note="Behavioral similarity is an inferred pattern and does not establish common authorship."
            )
        )
    ...
    if cat_sig.status == SignalStatus.MATCH:
        signals.append(
            RelationshipSignal(
                ...
                certainty=SignalCertainty.CONTEXTUAL,
